...
# rows carry no key, so the differ patches them positionally. jsbench_keyed.roc
# is the same app with a `key` on every row. The two brackets are separate apps
# rather than one app branching on a flag, so neither binary carries the other's
# code or tests a flag per row.
...
# so an unchanged row skips render and diff at the cost of one thunk. Nothing
# wraps the region, so the differ pairs rows by position.
render_row : Row, U64 -> Html(Msg)
render_row = |row, selected|
	Html.lazy2(row_view, row, selected == row.id)
