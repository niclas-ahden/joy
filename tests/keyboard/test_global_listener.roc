...
## Keyboard.add_global_listener! with empty filter fires on any keypress.
...
main! = |_args|
    TestEnvironment.with!(|worker_url|
        ...
        last_key = Playwright.text_content!(page, "#last-key")?
        Assert.eq(last_key, "a") ? WrongKey(last_key)

        count = Playwright.text_content!(page, "#event-count")?
        # At least 1 event from the AnyKey listener
        Assert.true(Str.to_i64(count) |> Result.with_default(0) >= 1) ? NoEvents(count)
