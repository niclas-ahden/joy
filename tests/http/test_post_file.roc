...
## Http.post_file! sends a POST with a browser File body and custom headers.
main! : List Arg.Arg => Result {} _
main! = |_args|
    TestEnvironment.with!(|worker_url|
        { browser, page } = Playwright.launch_page!(Chromium)?
        Playwright.navigate!(page, "$(worker_url)/http")?
        Playwright.wait_for!(page, "#btn-post-file", Visible)
        |> Result.map_err(|e| WasmDidNotLoad(Inspect.to_str(e)))?

        # Select a file first so file_id 1 exists
        Playwright.set_input_files!(page, "#file-input", Buffers([{
            name: "test_upload.txt",
            mime_type: "text/plain",
            buffer: Str.to_utf8("file content for post"),
        }]))?

        Playwright.click!(page, "#btn-post-file")?
        ...
        Assert.true(Str.contains(response, "post-file-value")) ? MissingHeader(response)
        # Verify body was received (non-zero size)
        Assert.true(!(Str.contains(response, "\"body_size\": 0"))) ? EmptyBody(response)
