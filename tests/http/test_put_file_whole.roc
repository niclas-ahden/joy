...
main! = |_args|
    TestEnvironment.with!(|worker_url|
        ...
        Assert.true(Str.contains(response, "entire file content")) ? WrongBody(response)
        Assert.true(Str.contains(response, "put-file-value")) ? MissingHeader(response)
