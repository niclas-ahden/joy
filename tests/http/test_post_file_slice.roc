...
## Http.post_file! with Slice sends only a byte range, not the whole file.
main! : List Arg.Arg => Result {} _
main! = |_args|
    TestEnvironment.with!(|worker_url|
        { browser, page } = Playwright.launch_page!(Chromium)?
        Playwright.navigate!(page, "$(worker_url)/http")?
        Playwright.wait_for!(page, "#btn-post-file-slice", Visible)
        |> Result.map_err(|e| WasmDidNotLoad(Inspect.to_str(e)))?

        # "hello world!" is 12 bytes. Slice(start: 2, len: 5) sends "llo w"
        ...
        Playwright.click!(page, "#btn-post-file-slice")?
        ...
        Assert.true(Str.contains(response, "llo w")) ? WrongBodyContent(response)
