...
## Http.post_file! with a file_id that doesn't exist fires an error event, not a crash.
...
main! = |_args|
    TestEnvironment.with!(|worker_url|
        ...
        # Click POST file WITHOUT selecting a file first — file_id 1 doesn't exist
        ...
        # Should receive an error, not crash
        Assert.true(Str.contains(response, "not found") || Str.contains(response, "err")) ? ExpectedError(response)
