...
## Time.debounce! resets the timer on repeated calls, firing only once after the last call.
main! : List Arg.Arg => Result {} _
main! = |_args|
    TestEnvironment.with!(|worker_url|
        { browser, page } = Playwright.launch_page!(Chromium)?
        Playwright.navigate!(page, "$(worker_url)/time")?
        Playwright.wait_for!(page, "#btn-debounce", Visible)
        |> Result.map_err(|e| WasmDidNotLoad(Inspect.to_str(e)))?

        # Call debounce 3 times rapidly (150ms timer, 50ms between clicks)
        Playwright.click!(page, "#btn-debounce")?
        _ = Sleep.millis!(50)
        Playwright.click!(page, "#btn-debounce-again")?
        _ = Sleep.millis!(50)
        Playwright.click!(page, "#btn-debounce-again")?

        # Wait for debounce to settle (150ms after last call + margin)
        _ = Sleep.millis!(400)

        # Should have fired exactly once despite 3 calls
        count = Playwright.text_content!(page, "#event-count")?
        Assert.eq(count, "1") ? FiredMoreThanOnce("expected 1, got $(count)")

        last_event = Playwright.text_content!(page, "#last-event")?
        Assert.eq(last_event, "debounce") ? WrongEvent(last_event)
