...
## Time.after! fires exactly once after the delay.
...
main! = |_args|
    TestEnvironment.with!(|worker_url|
        ...
        # Wait for the timer to fire (100ms delay + margin)
        _ = Sleep.millis!(300)

        last_event = Playwright.text_content!(page, "#last-event")?
        Assert.eq(last_event, "after") ? EventDidNotFire(last_event)

        count = Playwright.text_content!(page, "#event-count")?
        Assert.eq(count, "1") ? FiredMoreThanOnce(count)

        # Wait more to ensure it doesn't fire again
        _ = Sleep.millis!(300)
        count_after = Playwright.text_content!(page, "#event-count")?
        Assert.eq(count_after, "1") ? FiredAgain(count_after)
