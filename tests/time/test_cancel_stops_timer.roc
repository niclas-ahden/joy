...
## Time.cancel! stops a repeating interval from firing further.
...
main! = |_args|
    TestEnvironment.with!(|worker_url|
        ...
        count_at_cancel = Playwright.text_content!(page, "#event-count")?

        # Wait and verify no more fires
        _ = Sleep.millis!(400)
        count_after = Playwright.text_content!(page, "#event-count")?

        Assert.eq(count_at_cancel, count_after) ? StillFiring("at cancel: $(count_at_cancel), after wait: $(count_after)")
