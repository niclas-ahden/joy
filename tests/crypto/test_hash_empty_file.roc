...
main! = |_args|
    TestEnvironment.with!(|worker_url|
        ...
        # Verify zero chunk events
        chunk_count = Playwright.text_content!(page, "#chunk-count")?
        Assert.eq(chunk_count, "0") ? ExpectedZeroChunks(chunk_count)

        # Verify done event has total_chunks: 0
        done_result = Playwright.text_content!(page, "#done-result")?
        Assert.true(Str.contains(done_result, "\"total_chunks\":0")) ? WrongTotalChunks(done_result)
