...
## Reading with a non-zero `start` returns the requested byte range, not the
## head (start=2, len=2 over "hello" yields bytes 108,108 = "ll").
main! : List Arg.Arg => Result {} _
main! = |_args|
    TestEnvironment.with!(|worker_url|
        { browser, page } = Playwright.launch_page!(Chromium)?
        Playwright.navigate!(page, "$(worker_url)/file?start=2&len=2")?
        ...
        Assert.eq(byte_count, "2") ? WrongByteCount(byte_count)

        bytes = Playwright.text_content!(page, "#bytes")?
        Assert.eq(bytes, "108,108") ? WrongBytes(bytes)
