...
## Reading from offset 0 returns the file's leading bytes, and a `len` past the
## end of the file is clamped to what's available (here len=12 over a 5-byte
## file yields 5 bytes).
main! : List Arg.Arg => Result {} _
main! = |_args|
    TestEnvironment.with!(|worker_url|
        { browser, page } = Playwright.launch_page!(Chromium)?
        Playwright.navigate!(page, "$(worker_url)/file?start=0&len=12")?
        ...
        |> Result.map_err(|e| ReadDidNotComplete(Inspect.to_str(e)))?

        byte_count = Playwright.text_content!(page, "#byte-count")?
        Assert.eq(byte_count, "5") ? WrongByteCount(byte_count)

        bytes = Playwright.text_content!(page, "#bytes")?
        Assert.eq(bytes, "104,101,108,108,111") ? WrongBytes(bytes)
