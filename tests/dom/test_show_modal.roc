...
## DOM.show_modal! opens a <dialog> element.
...
main! = |_args|
    TestEnvironment.with!(|worker_url|
        ...
        visible_before = Playwright.is_visible!(page, "#dialog-content")?
        Assert.eq(visible_before, Bool.false) ? DialogVisibleBeforeShow
        ...
        Assert.eq(status, "opened") ? WrongStatus(status)
